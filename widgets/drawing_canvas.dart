...
class DrawingPath extends DrawingElementWithPoints {
  ...
  @override
  bool contains(Offset point) { return false; }
  @override
  void move(Offset delta) {
...
class DimensionLine extends DrawingElementWithPoints {
  ...
  @override
  Map<String, dynamic> toJson() => {
    'type': 'dimension', 'id': id, 'start': {'dx': start.dx, 'dy': start.dy}, 'end': {'dx': end.dx, 'dy': end.dy},
    'paint': {'color': paint.color.value, 'strokeWidth': paint.strokeWidth},
  };
...
class DrawingText extends DrawingElement {
  ...
  @override
  void draw(Canvas canvas, Size size) {
    final textSpan = TextSpan( text: text, style: TextStyle( color: paint.color, fontSize: 16, fontWeight: FontWeight.bold));
    _textPainter = TextPainter( text: textSpan, textDirection: ui.TextDirection.ltr);
    _textPainter!.layout(minWidth: 0, maxWidth: size.width);
    final adjustedPosition = Offset(
      math.min(position.dx, size.width - _textPainter!.width),
      math.min(position.dy, size.height - _textPainter!.height),
    );
  ...
  bool contains(Offset point) {
    ...
    final adjustedPosition = Offset(
      math.min(position.dx, _textPainter!.width - _textPainter!.width),
      math.min(position.dy, _textPainter!.height - _textPainter!.height),
    );
    final rect = Rect.fromLTWH(adjustedPosition.dx, adjustedPosition.dy, _textPainter!.width, _textPainter!.height);
