...
class PrintPreviewScreen extends StatelessWidget {
  ...
  pw.Widget _buildSingleSetContentArea(pw.Context context, KochuhyoData data, pw.Font ttfBold, pw.Font ttfRegular) {
    ...
    final titleTextStyle = pw.TextStyle(font: ttfBold, fontSize: baseFontSize * 1.5); 
    ...
    const double drawingHeight = 1.5 * PdfPageFormat.cm;
    ...
    pw.Widget _buildDimensionTable(String title, List<Map<String, String>> items, {bool isCompact = false, bool thicknessOnly = false}) {
      if (items.isEmpty && !title.contains("荷重")) return pw.Container();
      if (items.isEmpty && title.contains("荷重") && data.loadCalculationMethod == "等分布荷重" && data.allowableLoadUniform.isNotEmpty && data.allowableLoadUniform != "計算不可") {
      } else if (items.isEmpty && title.contains("荷重")) {
         return pw.Container();
      }

      List<pw.Widget> headerCells = [
        pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text('項目名', style: boldTextStyle)),
      ];
      if (!thicknessOnly) {
        headerCells.addAll([
          pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text('長さ', style: boldTextStyle)),
          pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text('幅', style: boldTextStyle)),
        ]);
      }
      headerCells.add(pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text('厚さ', style: boldTextStyle)));
      if (!thicknessOnly) {
        headerCells.add(pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text('数', style: boldTextStyle)));
      }
      ...
      return pw.Column(
        ...
        children: [
          pw.Text(title, style: headerTextStyle),
          pw.Table(
            ...
            children: [
              ...
              ...items.map((item) {
                List<pw.Widget> itemCells = [
                  pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text(item['項目名']!, style: mainTextStyle)),
                ];
                if(!thicknessOnly) {
                  itemCells.addAll([
                    pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text(item['長さ']!, style: mainTextStyle)),
                    pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text(item['幅']!, style: mainTextStyle)),
                  ]);
                }
                itemCells.add(pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text(item['厚さ']!, style: mainTextStyle)));
                if(!thicknessOnly) {
                  itemCells.add(pw.Padding(padding: const pw.EdgeInsets.all(tableCellPadding), child: pw.Text(item['本数']!, style: mainTextStyle)));
                }
    ...
    pw.Widget _buildDrawing(Uint8List? imageBytes, String placeholder) {
      ...
      return pw.Container(height: drawingHeight, margin: pw.EdgeInsets.only(bottom: sectionSpacing), width: double.infinity, decoration: pw.BoxDecoration(border: pw.Border.all(color: PdfColors.grey)), child: pw.Center(child: pw.Text(placeholder, style: mainTextStyle)));
    }

    pw.Widget _buildBasicInfoRow(List<pw.Widget> children) {
      return pw.Row(
        mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, 
        crossAxisAlignment: pw.CrossAxisAlignment.start, // 上揃えに変更
        children: children.map((e) => pw.Expanded(child: pw.Padding(
            padding: pw.EdgeInsets.only(right: oneCm * 0.1), //アイテム間の右パディングを少し調整
            child: e
        ))).toList(),
      );
    }

    // ★★★ エラー修正: _buildBasicInfoItem の定義に labelWidth を追加し、 crossAxisAlignment と textBaseline を削除 ★★★
    pw.Widget _buildBasicInfoItem(
      String label, 
      String value, {
      pw.TextStyle? valueStyle, 
      bool isMaterial = false, 
      double labelWidth = 40, // デフォルト値
    }) {
      return pw.Row(
          mainAxisSize: pw.MainAxisSize.min,
          crossAxisAlignment: pw.CrossAxisAlignment.start, // baseline が使えないので start に変更
          children: [
            pw.SizedBox( // ラベルの幅を指定
              width: labelWidth, 
    ...
    return pw.Column(
      ...
      children: [
        ...
        _buildBasicInfoRow([
          _buildBasicInfoItem('工番', data.kobango, labelWidth: 15 * PdfPageFormat.mm), // mm単位で指定
        ...
        _buildBasicInfoRow([
          ...
          pw.Expanded(child: pw.Container()), // 残りのスペースを埋めるためのダミー
          pw.Expanded(child: pw.Container()),
        ]),
        pw.SizedBox(height: sectionSpacing),
        pw.Divider(height: sectionSpacing, thickness: 0.5),
        dimensionsTable,
        pw.Divider(height: sectionSpacing, thickness: 0.5),
        pw.SizedBox(height: sectionSpacing),
        ...
        pw.Flexible( 
          ...
          child: pw.Row(
            ...
            children: [
              pw.Expanded( 
                ...
                child: pw.ListView( 
                  children: [
                    ...
                    pw.Text('計算方法: ${data.loadCalculationMethod}', style: mainTextStyle),
                    if (data.loadCalculationMethod == '2点集中荷重')
                        pw.Text(' > 詳細: ${data.twoPointLoadDetails}\n > 最終荷重: ${data.finalAllowableLoad}', style: mainTextStyle),
                    if (data.loadCalculationMethod != '2点集中荷重' && data.allowableLoadUniform.isNotEmpty && data.allowableLoadUniform != "計算不可")
                        pw.Text(' > 許容荷重: ${data.allowableLoadUniform}', style: mainTextStyle),
                    pw.SizedBox(height: sectionSpacing),
              ...
              pw.SizedBox(width: sectionSpacing * 3), 
              pw.Expanded( 
                flex: 1,
                child: pw.ListView( 
                  children: [
                    _buildDimensionTable('側・妻', _createTableItems(gawaTsumaSourceItems), isCompact: true),
                     pw.Text('かまち種類: ${data.kamachiType}', style: mainTextStyle),
                    _buildDrawing(data.gawaTsumaImageBytes, '側・妻図面なし'),
                    _buildDimensionTable('天井', _createTableItems(tenjoSourceItems), thicknessOnly: true, isCompact: true),
                    _buildDimensionTable('追加部材', _createTableItems(additionalPartsSourceItems), isCompact: true),
                  ]
                ),
              ),
            ],
